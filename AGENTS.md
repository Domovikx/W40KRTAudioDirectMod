...
- **Релизы — по тегам:** каждый релиз — это тег `vX.Y.Z` на master. Пользователи качают архив по тегу: `https://github.com/Domovikx/W40KRTAudioDirectMod/archive/refs/tags/vX.Y.Z.zip`
- **Рабочая ветка:** `feature/YYYY-MM-DD` (по дате начала). После завершения работы — мерж в master, тег, push.
- **Ветки:** только `master` (релизный) + одна активная feature-ветка. Старые ветки удалять.
- **Download ZIP:** `.gitattributes` исключает dev-файлы через `export-ignore`. Пользователь получает чистый архив с `Info.json`, DLL и `Localization/ruRU/`.
- **DLL трекается в git** (нужен для `export-ignore` архива). После `compile.bat` — коммитить новый DLL.
